## Functions for assessing the core language for a body of text,
## and generating a list of words that should not be included in the core
...
class RawCorpus(object):
	...
	def __init__(self, file_name, delimiter='\t', word_sep='|',
		...
		self.logger.info('Found %s texts in %s', self.num_docs,
			self.file_name)
...
def create_corebody(text_file, new_filename=None, delimiter='\t',
	word_sep='|', min_docnum=0, max_docnum=1.0, tokens_limit=None,
	encoding='utf-8'):
